...
class Attention(nn.Module):
    ...
    def forward(self, k, q):
        ...
        # output_aspect = torch.bmm(score * aspect_mask.unsqueeze(dim=1) / aspect_mask.sum(dim=-1).unsqueeze(-1), kx)
        output = torch.cat(torch.split(output, mb_size, dim=0), dim=-1)
        output = self.proj(output)
        output = self.dropout(output)
        # output = self.proj(output_aspect)
        # output = self.dropout(output_aspect)
        return output, score