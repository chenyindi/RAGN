import numpy as np
import torch
import torch.nn as nn
from torch.nn import CrossEntropyLoss
from transformers.modeling_bert import BertForTokenClassification, BertPooler
import torch.nn.functional as F


class BERT_NER(BertForTokenClassification):

    def __init__(self, bert_base_model):
        super(BERT_NER, self).__init__(config=bert_base_model.config)
        self.bert4ner = bert_base_model
        # self.bilstm = nn.LSTM(768, 768 // 2, bidirectional=True, batch_first=True)
        # self.cnn = nn.Conv1d(768, 768, kernel_size=3, padding=1)
        # self.attn = nn.MultiheadAttention(768, 12)
        # self.ln = nn.LayerNorm(768)  # 768 is the input size

    def forward(self, input_ids,
                token_type_ids=None,
                attention_mask=None,
                valid_ids=None,
                label_mask=None,
                labels=None
                ):

        outputs = self.bert4ner(input_ids, token_type_ids, attention_mask)
        sequence_output = outputs[0]
        batch_size, max_len, feat_dim = sequence_output.shape
        valid_output = torch.zeros(batch_size, max_len, feat_dim, dtype=torch.float32, device='cuda')
        for i in range(batch_size):
            jj = -1
            for j in range(max_len):
                if valid_ids[i][j].item() == 1:
                    jj += 1
                    valid_output[i][jj] = sequence_output[i][j]
        # 取所有有效字的均值
        valid_feature = valid_output.mean(dim=1)
        sequence_output = self.dropout(valid_output)
        ate_logits = self.classifier(sequence_output)
        # lstm_output, _ = self.bilstm(sequence_output)
        # attn_output, _ = self.attn(sequence_output.permute(1, 0, 2), sequence_output.permute(1, 0, 2),
        #                            sequence_output.permute(1, 0, 2))
        # attn_output = attn_output.permute(1, 0, 2)
        # cnn_output = self.cnn(lstm_output.permute(0, 2, 1))  # use a cnn to get output
        # cnn_output = cnn_output.permute(0, 2, 1) # torch.Size([16, 128, 768])
        # residual_output = lstm_output + cnn_output
        # norm_output = self.ln(residual_output)
        # ate_logits = self.classifier(norm_output)

        return valid_feature, ate_logits, sequence_output
        # return outputs[1], ate_logits, sequence_output

        # if labels is not None:
        #     criterion_ate = CrossEntropyLoss(ignore_index=0)
        #     loss_ate = criterion_ate(ate_logits.view(-1, self.num_labels), labels.view(-1))
        #     return loss_ate
        # else:
        #     return ate_logits
