    ...
    if embedding_type == 'glove' or embedding_type == 'elmo':
        ...
        if args.pure_bert:
            ...
            inputs = {'input_ids': batch[0],
                      ...
                      # 是这里的问题，导致TypeError: forward() got an unexpected keyword argument 'ncon_num_ids'
                      # 尽管model的forward中没有将ncon_num_ids作为形参
                      'ncon_num_ids': batch[20] # 这里是能取到值的，那到底要怎么样才能让forward用呢？突然把ncon_num_ids加到forward里又能读取到了
                      }
            # print('=========================dep_tags: batch[6]:', batch[20])
            labels = batch[10]
            ner_labels = batch[15]
...
def train(args, train_dataset, model, test_dataset):
    ...
    # print('--------------train_dataset:', train_dataset.__dict__)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # loss = 1 * loss_cls + 1 * loss_ner  最终损失函数
