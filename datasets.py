...
def get_dataset(dataset_name, args=None):
    ...
    print('read_sentence_depparsed(ds_train[dataset_name]):', read_sentence_depparsed(ds_train[dataset_name]))
        ...
        if as_start <= i < as_end:
            ...
            for asp in range(as_start, as_end):
                ...
                if dis < min_dis:
                    min_dis = dis
            flag = True
            if min_dis > max_hop and add_non_connect:
                flag = False
                import warnings
...
def get_rolled_and_unrolled_data(input_data, args):
    ...
    for e in input_data:
        ...
        for i in range(len(e['aspect_sentiment'])):
            ...
            # 后面重新生成
            # print('1***************dep_tag:',dep_tag)
            # # print('1***************dep_idx:',dep_idx)
            # # print('1***************dep_dir:',dep_dir)
            # length = len(dep_tag)
            # for tag in dep_tag:
            #     if tag.startswith('ncon_'):
            #         num = int(tag.split('_')[1])
            #     elif tag != '<pad>' and tag != 'non-connect':
            #         num = 1
            #     elif tag == 'non-connect':
            #         num = length
            #     elif tag == '<pad>':
            #         num = 80
            #     else:
            #         continue
            #     ncon_nums.append(num)
            # print('1***************ncon_nums', ncon_nums)
            # print('1***************len(ncon_nums)', len(ncon_nums))
            ...
            if len(dep_tag) == 0:
                ...
                if len(dep_tag) == 0:  # for debugging
                    ...
                    zero_dep_counter -= 1
            # print('aaaaaaaaaaadep_tag:', dep_tag)

            dep_tags.append(dep_tag)
            dep_index.append(dep_idx)
            dep_dirs.append(dep_dir)
            # print('2***************dep_tags:',dep_tags)
            # print('2***************len(dep_tags):',len(dep_tags))
            # print('2***************len(dep_tag):',len(dep_idx))
            # print('2***************dep_idx:',dep_idx)
            # print('2***************dep_dir:',dep_dir)
            # print('2***************len(dep_dir):',len(dep_dir))
            length = len(dep_tag)
            ncon_num = []
            for tag in dep_tag:
                if tag.startswith('ncon_'):
                    ...
                    continue
                ncon_num.append(num)
            ncon_nums.append(ncon_num)
            # print('2***************ncon_nums：', ncon_nums)
            # print('2***************len(ncon_nums)：', len(ncon_nums))
            ...
            all_unrolled.append(
                {'sentence': e['tokens'], 'tags': e['tags'], 'pos_class': pos_class, 'aspect': aspect,
                 ...
                 'dependencies': e['dependencies'], 'ner_tokens': e['ner_tokens'], 'ncon_num': ncon_num})
            # print('oooooooooooooooooooooall_unrolled:', all_unrolled)
        ...
        all_rolled.append(
            {'sentence': e['tokens'], 'tags': e['tags'], 'pos_class': pos_class, 'aspects': aspects,
             ...
             'from': froms, 'to': tos, 'dep_tags': dep_tags, 'dep_index': dep_index, 'dependencies': e['dependencies']
                , 'ner_tokens': e['ner_tokens'], 'ncon_nums': ncon_nums})
        ...
        if len(e['aspect_sentiment']) and len(set(map(lambda x: x[1], e['aspect_sentiment']))) > 1:
            mixed_rolled.append(
                {'sentence': e['tokens'], 'tags': e['tags'], 'pos_class': pos_class, 'aspects': aspects,
                 ...
                 'dependencies': e['dependencies'], 'ncon_nums': ncon_nums})
...
class ASBA_Depparsed_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        items = e['dep_tag_ids'], \
            ...
            e['ncon_num_ids'], e['adj'], e['src_mask'], e['context_attention_mask']
        # print('e[ncon_num_ids]:', e['ncon_num_ids']) # 打印的是参差不齐的列表
        # print('e[adj]:', e['adj']) 有值
        if self.args.embedding_type == 'glove':
            # e是数据集中的一个样本，e['sentence_ids']对应的是该样本的sentence_ids
    ...
    def convert_features_bert(self, i, max_len=128, direct=False, parseadj=False, pad_id=0):
        """
        BERT features.
        ...
        input_aspect_ids = self.args.tokenizer.convert_tokens_to_ids(
            aspect_tokens)
        ##########
        adj = np.ones(max_len) * pad_id
        if parseadj:
            from absa_parser import headparser
            # * adj
            headp, syntree = headparser.parse_heads(self.data[i]['sentence'])
            adj = softmax(headp[0])
            ...
            adj = np.pad(adj, (0, max_len - adj.shape[0]), 'constant')

        # print('convert_features_bert==========================================adj:', adj)
    ...
    def convert_features_bert_with_len(self, i, max_len=128, use_spc=False, parseadj=False, direct=False, pad_id=0):
        """
        BERT features.
        convert sentence to feature.
        ...
        # print('before------self.data[i][sentence]：', self.data[i]['sentence']) # 例如 ['owner', 'is', 'pleasant', 'and', 'entertaining', '.']
        for idx, word in enumerate(self.data[i]['sentence']):
            word_tokens = self.args.tokenizer.tokenize(word)
            token_idx = len(tokens)
            tokens.extend(word_tokens)
        ...
        #  tokens:   [CLS] the dog is hairy . [SEP]
        #  type_ids:   0   0   0   0  0     0   0
        # print('after------word_indexer：', word_indexer)
        context_len = len(tokens)
        paddings = [0] * (128 - context_len)
        src_mask = [0] + [1] * context_len + [0] * (128 - context_len - 1)
        src_mask = np.asarray(src_mask, dtype='int64')
        # print('==================src_mask:', src_mask)
        context_attention_mask = [1] * context_len + paddings
        ...
        if parseadj:
            from absa_parser import headparser
            # * adj
            # print('===============================sentence:', self.data[i]['sentence'])
            headp, syntree = headparser.parse_heads(self.data[i]['sentence'])
            adj = softmax(headp[0])
            adj = np.delete(adj, 0, axis=0)
            adj = np.delete(adj, 0, axis=1)
            ...
            adj = adj + np.eye(adj.shape[0])
            adj = np.pad(adj, (0, max_len - adj.shape[0]), 'constant')
        # print('convert_features_bert_with_len==========================================adj:', adj)
        # if not use_spc:
        ...
        if self.args.pure_bert:
            ...
            self.data[i]['adj'] = adj
            self.data[i]['src_mask'] = src_mask
            self.data[i]['context_attention_mask'] = context_attention_mask
            if use_spc:
    ...
    def convert_features(self):
        ...
        for i in range(len(self.data)):
            ...
            self.data[i]['ncon_num_ids'] = [idx
                                            for idx in self.data[i]['ncon_num']]
            # print('=============================：', self.data[i]['ncon_num_ids']) # 这里也是能取到值的，参差不齐的列表
            self.data[i]['adj'] = [idx for idx in self.data[i]['adj']]
            self.data[i]['src_mask'] = [idx for idx in self.data[i]['src_mask']]
            self.data[i]['context_attention_mask'] = [idx for idx in self.data[i]['context_attention_mask']]
            # print('=============================：', self.data[i]['adj']) # # 这里也是能取到值
...
def my_collate(batch):
    ...
    '''
    sentence_ids, aspect_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids, ncon_num_ids, adj, src_mask, context_attention_mask= zip(
    ...
    ncon_num_ids = pad_sequence(ncon_num_ids, batch_first=True, padding_value=80)
    adj = pad_sequence(adj, batch_first=True, padding_value=0)
    src_mask = pad_sequence(src_mask, batch_first=True, padding_value=0)
    context_attention_mask = pad_sequence(context_attention_mask, batch_first=True, padding_value=0)
    ...
    ncon_num_ids = ncon_num_ids[sorted_idx]
    adj = adj[sorted_idx]
    src_mask = src_mask[sorted_idx]
    context_attention_mask = context_attention_mask[sorted_idx]

    return sentence_ids, aspect_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids, ncon_num_ids, adj, src_mask, context_attention_mask
...
def my_collate_pure_bert(batch):
    ...
    input_cat_ids, segment_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids, ncon_num_ids, adj, src_mask, context_attention_mask = zip(
        *batch)  # from Dataset.__getitem__()
    ...
    return input_cat_ids, segment_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids, ncon_num_ids, adj, src_mask, context_attention_mask
...
def my_collate_bert(batch):
    ...
    input_ids, word_indexer, input_aspect_ids, aspect_indexer, input_cat_ids, segment_ids, dep_tag_ids, pos_class, \
        ...
        ner_mask, ner_input_mask, ner_segment_ids, ncon_num_ids, adj, src_mask, context_attention_mask= zip(*batch)
    text_len = torch.tensor(text_len)
    aspect_len = torch.tensor(aspect_len)
    sentiment = torch.tensor(sentiment)
    ...
    # print('before-------input_aspect_ids', input_aspect_ids)
    ...
    # print('final-------input_aspect_ids', input_aspect_ids)
    ...
    return input_ids, word_indexer, input_aspect_ids, aspect_indexer, input_cat_ids, segment_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids, ner_labels, ner_valid, ner_mask, ner_input_mask, ner_segment_ids, ncon_num_ids, adj, src_mask, context_attention_mask
